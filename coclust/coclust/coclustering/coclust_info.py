...
class CoclustInfo(BaseNonDiagonalCoclust):
    ...
    def __init__(self, n_row_clusters=2, n_col_clusters=2, init=None, missing_indexes=None,
    ...
    def _fit_single(self, X, random_state, y=None):
        ...
        while change and n_iters > 0:
            ...
            if (self.missing is not None):
                print('Step 2++, Iteration :',21-n_iters)
            ...
            # Imputation 
            if self.missing is not None:
                print('Step 4++, Iteration :',21-n_iters+1)
                self._impute(X, p_kl, Z=Z, W=W)
    ...
    def _impute(self, X, p_kl, Z, W):
        ...
        """
        ## Store number of N by co-cluster 
        coclust_n = np.copy(p_kl.todense()).astype(int)
